...
export default function Home() {
  ...
  const handleAddWenn = () => {
    const elArr = Array.from(elementsWenn)
    const id = elementsWenn.length + 1
    elArr.push(<FormLineIf id={id} elArr={elementsWenn} setEl={setElementsWenn} fields={fieldsForRuleIf} />)
    setElementsWenn(elArr)
    setIdWenn(idWenn + 1)
  }

  const handleAddDann = () => {
    const elArr = Array.from(elementsDann)
    const id = elementsDann.length + 1
    elArr.push(<FormLineThan id={id} elArr={elementsDann} setEl={setElementsDann} fields={fieldsForRuleThen} />)
